...
#define DERR(...)       printf(__VA_ARGS__)
...
int16_t SonyOpen(uint32_t pb, uint32_t dce, uint32_t status)
{
	...
	// Install drives
        ...
        // Set up drive status based on actual disk size
        // 400K = single sided GCR, 800K = double sided GCR
        int is_800k = (info->size > 400*1024);
        int is_large = (info->size > 2*1024*1024); // > 2MB

        WriteMacInt16(info->status + dsQType, is_large ? 1 : sony);
        WriteMacInt8(info->status + dsInstalled, 1);
        WriteMacInt8(info->status + dsSides, is_800k ? 0xff : 0);
        WriteMacInt8(info->status + dsTwoSideFmt, is_800k ? 0xff : 0);
        WriteMacInt8(info->status + dsMFMDrive, 0);     // 0 = 400/800K GCR drive
        WriteMacInt8(info->status + dsMFMDisk, 0);
        
        // Report 32-bit block count for large images to prevent OS division-by-zero exceptions
        uint32_t blocks = info->size / 512;
        WriteMacInt16(info->status + 18, blocks & 0xFFFF);         // dsDriveSize (Low word)
        WriteMacInt16(info->status + 20, (blocks >> 16) & 0xFFFF); // dsDriveS1   (High word)

        DERR("Disc: size=%u -> %s format, is_large=%d, blocks=%u\n", info->size, is_800k ? "800K" : "400K", is_large, blocks);
...
*/
		...
		case 7:			// Eject
			if (ReadMacInt8(info->status + dsDiskInPlace) > 0) {
                                ...
                                DERR("DISC: EJECT requested! PC=0x%06x SP=0x%06x D0=0x%08x\n", pc, sp, d0);
                                DERR("DISC: pb=0x%06x dce=0x%06x diskInPlace=%d\n",
                                     pb, dce, ReadMacInt8(info->status + dsDiskInPlace));
//				SysEject(info->fh);
